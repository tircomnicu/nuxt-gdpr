  ...
  methods: {
    emitInput: function emitInput(newInput) {
      if (this.range && this.value) {
        this.value.length === 2 ? this.$emit('input', [newInput]) : this.$emit('input', [].concat(_toConsumableArray(this.value), [newInput]));
        return;
      }
    ...
    genDateTable: function genDateTable() {
      ...
      var proxyValue = this.value;

      if (this.range && this.value && this.value.length === 2) {
        proxyValue = [];

        var _map$sort = [this.value[0], this.value[1]].map(function (x) {
          return new Date("".concat(x, "T00:00:00+00:00"));
        }).sort(function (a, b) {
          return a > b ? 1 : -1;
        }),
            _map$sort2 = _slicedToArray(_map$sort, 2),
            rangeFrom = _map$sort2[0],
            rangeTo = _map$sort2[1];

        var diffDays = Math.ceil((rangeTo.getTime() - rangeFrom.getTime()) / (1000 * 60 * 60 * 24));

        for (var i = 0; i <= diffDays; i++) {
          var current = new Date(+rangeFrom + i * 864e5);
