...
const commands = {
  start: () => new Promise(function (resolve) { resolve(require('./cli-start.js')); }),
  dev: () => new Promise(function (resolve) { resolve(require('./cli-dev.js')); }),
  build: () => new Promise(function (resolve) { resolve(require('./cli-build.js')); }),
  generate: () => new Promise(function (resolve) { resolve(require('./cli-generate.js')); }),
  help: () => new Promise(function (resolve) { resolve(require('./cli-help.js')); })
};
...
async function _import (modulePath) {
  for (const mp of [
    ...
    try {
      return await new Promise(function (resolve) { resolve(_interopNamespace(require(mp))); })
    } catch (e) {
...
class NuxtCommand extends Hookable {
  ...
  async run () {
    ...
    if (this.argv['force-exit']) {
      ...
      index.forceExit(this.cmd.name, forceExitByUser ? false : index.forceExitTimeout);
      if (forceExitByUser) {
        return
      }
    }
  ...
  _getHelp () {
    ...
    const _opts = options.map(([option, description]) => {
      const i = index.indent(maxOptionLength + index.optionSpaces - option.length);
      return index.foldLines(
        option + i + description,
        index.startSpaces + maxOptionLength + index.optionSpaces * 2,
        index.startSpaces + index.optionSpaces
      )
    }).join('\n');

    const usage = index.foldLines(`Usage: nuxt ${this.cmd.usage} [options]`, index.startSpaces);
    const description = index.foldLines(this.cmd.description, index.startSpaces);
    const opts = index.foldLines(`Options:`, index.startSpaces) + '\n\n' + _opts;

    let helpText = index.colorize(`${usage}\n\n`);
    if (this.cmd.description) {
      helpText += index.colorize(`${description}\n\n`);
    }
    if (options.length) {
      helpText += index.colorize(`${opts}\n\n`);
    }
